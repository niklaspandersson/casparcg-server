...
struct screen_consumer_vk
{
    ...
    std::vector<core::const_frame> in_flight_frames_;
  ...
  public:
    ...
    void run()
    {
        ...
        accelerator::vulkan::queue_request req;
        req.required_flags   = vk::QueueFlagBits::eGraphics;
        req.prefer_exclusive = true;
        req.label            = "screen-present";
        queue_               = device_->acquire_queue(req);

        if (!queue_) {
            // Relax exclusivity preference and try again, to allow sharing a graphics queue when the present queue is
            // not separate (e.g. on integrated GPUs).
            req.prefer_exclusive = false;
            queue_               = device_->acquire_queue(req);
            if (!queue_) {
                CASPAR_THROW_EXCEPTION(caspar_exception()
                                       << msg_info("No graphics queue available for screen consumer"));
            }
        }
        auto queue = queue_->vk_queue();

        // The command_context owns the command pool (on the present queue's
        // family) + timeline + recycled command buffers. render_pipeline records
        // its own per-frame buffers from this pool.
        ...
        in_flight_frames_.resize(swapchain_->max_frames_in_flight());
        ...
        // before destroying the command pool and window. The device is idle, so
        // any composited textures we were still holding are safe to release.
        render_pipeline_.reset();
        swapchain_.reset();
        in_flight_frames_.clear();
        // render_pipeline_.reset() above waited for the device to be idle, so the
        // command pool owned by command_ctx_ has no in-flight buffers left.
        command_ctx_.reset();
        queue_.reset();
        window_.reset();
    }
    ...
    void tick()
    {
        ...
        // empty frames), so a missing/foreign texture is unexpected — skip it.
        ...
        // Acquire the swapchain image first, so that bailing on an out-of-date
        // swapchain cannot strand an in-flight ownership transfer (below).
        ...
        uint32_t frame_slot  = 0;
        {
            ...
            image_index = swapchain_->acquire_next_image();
            ...
            // The slot's fence (just waited) guarantees the previous submit in
            // this slot has retired, so the texture it sampled can be released.
            frame_slot                    = swapchain_->frame_slot();
            in_flight_frames_[frame_slot] = in_frame;
        }

        // The draw waits on the renderer's completion of `src` (RAW). When our
        // present queue is on a different family than the renderer (e.g. one queue
        // per family), the EXCLUSIVE composited texture must additionally have its
        // ownership transferred from the renderer queue to ours: the renderer
        // records the release, we record the matching acquire, ordered by a binary
        // semaphore. This is done outside the queue lock because the acquire goes
        // through command_ctx_, which takes the queue's slot mutex internally —
        // taking it again under scoped_lock would deadlock. The CONCURRENT empty
        // texture has owner_family == ignored and is skipped.
        std::vector<accelerator::vulkan::completion_token> waits = src->read_dependencies();
        if (src->owner_family() != VK_QUEUE_FAMILY_IGNORED && src->owner_family() != queue_->family_index()) {
            auto transfer = device_->release_output(src, queue_);
            waits.push_back(accelerator::vulkan::acquire_texture(*command_ctx_,
                                                                 transfer,
                                                                 *src,
