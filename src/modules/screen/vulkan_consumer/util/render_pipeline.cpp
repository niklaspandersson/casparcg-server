...
struct render_pipeline::impl
{
    ...
    std::vector<vk::DescriptorSet> descriptor_sets_; // one per swapchain image
    ...
    // Transition `src` from the layout the renderer left it in to shader-read on
    // this (the present) queue. The cross-queue execution dependency on the
    // renderer is provided by the timeline-semaphore wait at submit time; this
    // barrier performs the layout transition + makes the image visible to the
    // fragment shader. Same-family assumption: no ownership-transfer barrier.
    void barrier_to_shader_read(vk::CommandBuffer cmdBuffer, vk::Image image, vk::ImageLayout src_layout)
    {
        if (src_layout == vk::ImageLayout::eShaderReadOnlyOptimal)
            return;
        ...
        b.srcAccessMask       = vk::AccessFlagBits2::eMemoryRead | vk::AccessFlagBits2::eMemoryWrite;
                ...
                vk::Fence                                                 fence)
    {
        ...
        // Bring the composited texture into shader-read layout on this queue.
        barrier_to_shader_read(cmdBuffer, vk::Image(src.id()), src_layout);
        ...
        // Waits: the swapchain image-available (binary) at color-output, plus the
        // renderer's completion of `src` (timeline) before the fragment shader
        // samples it. submit2 lets us mix binary and timeline semaphores.
        std::vector<vk::SemaphoreSubmitInfo> waits;
        waits.reserve(1 + wait_tokens.size());
        ...
        waits.push_back(image_available);

        for (const auto& token : wait_tokens) {
            if (!token)
                continue;
            vk::SemaphoreSubmitInfo w{};
            w.semaphore = token.timeline;
            w.value     = token.value;
        ...
        submitInfo.setWaitSemaphoreInfos(waits);
...
{
    impl_->render(
        src, src_layout, wait_tokens, image_index, frame_slot, params, wait_semaphore, signal_semaphore, fence);
}
