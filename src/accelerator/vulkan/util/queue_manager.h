...
// Threaded from release_texture()/release_buffer() to acquire_texture()/
// acquire_buffer() (see queue_transfer.h). Carries the binary semaphore that
// orders the release submit before the acquire submit, the two queue families
// involved, and the layout transition the acquire barrier must replay. Owns the
// semaphore via semaphore_owner — when both the release and acquire sides drop
// their retained references, vkDestroySemaphore is called.
...
class vulkan_queue final
{
  public:
    ...
    // Inter-queue ownership transfer is provided as free functions over
    // command_context — see release_texture/acquire_texture in queue_transfer.h.
