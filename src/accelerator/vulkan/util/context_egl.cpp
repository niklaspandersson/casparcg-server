...
struct device_context::impl
{
        ...
        , eglContext_(EGL_NO_CONTEXT)
    {
        ...
        if (!eglChooseConfig(eglDisplay_, configAttribs, &eglConfig, 1, &numConfigs)) {
            CASPAR_THROW_EXCEPTION(gl::vulkan_exception() << msg_info("Failed to initialize OpenGL: eglChooseConfig"));
        }

        if (!eglBindAPI(EGL_OPENGL_API)) {
            CASPAR_THROW_EXCEPTION(gl::vulkan_exception() << msg_info("Failed to initialize OpenGL: eglBindAPI"));
        }

        eglContext_ = eglCreateContext(eglDisplay_, eglConfig, EGL_NO_CONTEXT, NULL);
        if (eglContext_ == EGL_NO_CONTEXT) {
            CASPAR_THROW_EXCEPTION(gl::vulkan_exception() << msg_info("Failed to initialize OpenGL: eglCreateContext"));
        }

        if (!eglMakeCurrent(eglDisplay_, EGL_NO_SURFACE, EGL_NO_SURFACE, eglContext_)) {
            CASPAR_THROW_EXCEPTION(gl::vulkan_exception() << msg_info("Failed to initialize OpenGL: eglMakeCurrent"));
        }
    }
